...
protocol TimeOff {
    var vacationDays: Int {get set} //protocol variables need to be get/set or both
    func requestForVacation(_ days: Int) -> Bool
    }

extension TimeOff {
    //implementing the function from protocol in the extension
    func requestForVacation(_ days: Int) -> Bool {
        guard vacationDays >= days
            else {return false}
            return true
            }
        }
